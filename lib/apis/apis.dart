...
import 'package:flutter_trainning/model/movie_image.dart';
...
class ApiService {
  ...
  final String apiKey = 'api_key=e041848baaed8d48773c7c5e9ed6be4a';
  ...
  Future<List<Movie>> getSearch(String keyWord) async {
    try {
      final url = '$baseUrl/search/movie?$apiKey&"$keyWord"';
      final response = await _dio.get(url);
      var movies = response.data['results'] as List;
      List<Movie> movieList = movies.map((m) => Movie.fromJson(m)).toList();
      return movieList;
